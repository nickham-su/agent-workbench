<template>
  <div>
    ...
    <div v-if="expanded">
      <div v-if="loading" class="text-[color:var(--text-tertiary)]">Loading diff...</div>
      <div v-else-if="loadError" class="text-red-500">diff unavailable: {{ loadError }}</div>
      <template v-else-if="artifact">
        <div v-if="showCreateFileContent" class="rounded border border-[var(--border-color-secondary)] overflow-hidden">
          <div class="px-2 py-1 text-[color:var(--text-tertiary)] border-b border-[var(--border-color-secondary)]">
            New file content
          </div>
          <MonacoCodeViewer
        ...
        <div v-else-if="!canRenderDiff" class="text-[color:var(--text-tertiary)]">diff unavailable</div>
        ...
        <div v-if="truncatedHint" class="pt-1 text-[color:var(--text-tertiary)]">{{ truncatedHint }}</div>
      </template>
    </div>
  </div>
</template>
