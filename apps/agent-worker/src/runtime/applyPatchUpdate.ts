...
export function deriveNewContentFromChunks(params: {
  filePath: string;
  originalContent: string;
  chunks: UpdateFileChunk[];
}) {
  const originalLines = params.originalContent.split("\n");
  if (originalLines.length > 0 && originalLines[originalLines.length - 1] === "") {
    originalLines.pop();
  }

  const replacements = computeReplacements(originalLines, params.filePath, params.chunks);
  let nextLines = applyReplacements(originalLines, replacements);
...
function computeReplacements(originalLines: string[], filePath: string, chunks: UpdateFileChunk[]) {
...
function seekSequence(lines: string[], pattern: string[], start: number, endOfFile: boolean): number | null {
...
function linesMatch(lines: string[], pattern: string[], start: number, normalize: (value: string) => string) {
  for (let i = 0; i < pattern.length; i += 1) {
    if (normalize(lines[start + i] ?? "") !== normalize(pattern[i] ?? "")) {
      return false;
    }
  }
  return true;
