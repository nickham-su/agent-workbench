...
function createPreforkContext(
  ...
  const context: ToolExecutionContext = {
    ...
    pendingTool: {
      toolExecutionId: "execution-9",
      callPartId: "part-9",
      assistantMessageId: "message-9",
      status: "queued",
      toolName: "subtask",
      toolCallId: "call_1",
      args: {},
...
test("subtask prefork summary 透传 messages-context.system 到 one-shot 调用", async () => {
  ...
  assert.equal(
    (captured as { sessionId?: string } | null)?.sessionId,
    "sess_parent",
  );
...
test("subtask prefork shouldPrefork=false 跳过上下文摘要但仍按固定 95 启动", async () => {
  ...
  const apiClient = {
    async getSubtaskPreforkPlan(
      input: Parameters<AgentApiClient["getSubtaskPreforkPlan"]>[0],
    ) {
      thresholdPct = input.thresholdPct;
      return {
        shouldPrefork: false,
        thresholdPct: 95,
        parentLastResponseTotalTokens: 1,
        childContextWindowTokens: 10,
        thresholdTokens: 9,
      };
    },
    async getMessagesContext() {
      contextCalls += 1;
      return { headMessageId: null, system: "system", messages: [] };
    },
    async startSubtaskRun() {
      startCalls += 1;
  ...
  await provider.execute(
    "subtask",
    preforkArgs,
    createPreforkContext(apiClient),
  );
...
test("subtask prefork 摘要为空时不发送摘要字段并继续启动", async () => {
  let startInput: Parameters<AgentApiClient["startSubtaskRun"]>[0] | null =
    null;
  const apiClient = {
    async getSubtaskPreforkPlan() {
      return {
    ...
    async startSubtaskRun(
      input: Parameters<AgentApiClient["startSubtaskRun"]>[0],
    ) {
      startInput = input;
      return successfulStart();
    },
  ...
  class EmptySummaryProvider extends BuiltinToolProvider {
    protected override async generateSingleCallSummary(
      ...
      return { text: "   ", totalTokens: null };
    }
  }
  await new EmptySummaryProvider().execute(
    "subtask",
    preforkArgs,
    createPreforkContext(apiClient),
  );
...
test("subtask prefork 普通摘要错误固定脱敏 warning 后降级启动", async () => {
  ...
  try {
    ...
    class FailedSummaryProvider extends BuiltinToolProvider {
      protected override async generateSingleCallSummary(
        ...
        throw new Error("prompt TOKEN full result");
      }
    }
    await new FailedSummaryProvider().execute(
      "subtask",
      preforkArgs,
      createPreforkContext(apiClient),
    );
  ...
  assert.equal(
    String(warnings[0]?.[0]),
    "[agent-worker] subtask prefork summary failed; falling back to normal fork",
  );
...
for (const abortCase of [
  {
    label: "AbortError",
    error: Object.assign(new Error("summary stopped"), { name: "AbortError" }),
  },
  { label: "abort message", error: new Error("request aborted") },
  {
    label: "abort code",
    error: Object.assign(new Error("request stopped"), { code: "ABORT_ERR" }),
  },
]) {
  test(`subtask prefork ${abortCase.label} 不启动 child`, async () => {
    ...
    class AbortSummaryProvider extends BuiltinToolProvider {
      protected override async generateSingleCallSummary(
        ...
        throw abortCase.error;
      }
    }
    await assert.rejects(
      () =>
        new AbortSummaryProvider().execute(
          "subtask",
          preforkArgs,
          createPreforkContext(apiClient),
        ),
...
test("subtask prefork 成功时透传固定 threshold 与一致 preforkMeta", async () => {
  ...
  const capturedStartInput = startInput as
    Parameters<AgentApiClient["startSubtaskRun"]>[0] | null;
  assert.ok(capturedStartInput, "startSubtaskRun input should be captured");
  assert.deepEqual(capturedStartInput.preforkMeta, {
    thresholdPct: 95,
    parentLastResponseTotalTokens: 123,
    childContextWindowTokens: 456,
  });
...
test("subtask prefork 摘要成功后立即 abort 不启动 child", async () => {
  ...
  await assert.rejects(
    () =>
      new AbortAfterSummaryProvider().execute(
        "subtask",
        preforkArgs,
        createPreforkContext(apiClient, controller.signal),
      ),
    (err: unknown) => {
  ...
  class TestBuiltinProvider extends BuiltinToolProvider {
    ...
    protected override async prepareApplyPatch(params: {
      ...
      if (this.prepareCalls === 1) {
        throw new Error(
          "IO_RETRYABLE: add failed for /tmp/workspace/a.txt (EBUSY)",
        );
      }
      return {
        operations: [],
  ...
  const result = await provider.execute(
    ...
    },
    {
      profile: {
        provider: {},
        model: {},
        agent: { tools: ["apply_patch"], pluginTools: [], mcpServers: [] },
      },
      run: {
        workspaceId: "ws",
        sessionId: "sess",
...
test("apply_patch 不会对 legacy patch 做自动重试且失败文本不重复套壳", async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => {
      assert.match(
        String(err),
        /apply_patch verification failed: LEGACY_PATCH_FORMAT/,
      );
