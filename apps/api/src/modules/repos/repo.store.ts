...
export function insertRepo(db: Db, repo: RepoRecord) {
  db.prepare(
    `
      insert into repos (id, url, credential_id, created_at, updated_at, mirror_path, sync_status, sync_error, last_sync_at)
      values (@id, @url, @credentialId, @createdAt, @updatedAt, @mirrorPath, @syncStatus, @syncError, @lastSyncAt)
    `
  ).run(repo);
}
...
export function countWorkspacesReferencingRepo(db: Db, repoId: string) {
  const row = db.prepare(`select count(1) as cnt from workspaces where repo_id = ?`).get(repoId) as { cnt: number };
  return row.cnt;
}
