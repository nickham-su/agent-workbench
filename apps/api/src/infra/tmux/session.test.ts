...
test("tmux has-session 仅将明确 not-found 视为不存在", () => {
  ...
  assert.equal(classifyTmuxHasSessionResult({ ok: false, code: 1, stdout: "", stderr: "error connecting to /tmp/tmux-1001/default (no such file or directory)", timedOut: false }), "not_found");
});
...
test("tmux has-session 的 spawn、timeout、未知错误不可伪装为不存在", () => {
  for (const result of [
    ...
    { ok: false, code: 1, stdout: "", stderr: "error connecting to /tmp/tmux-1001/default (permission denied)", timedOut: false },
  ]) {
