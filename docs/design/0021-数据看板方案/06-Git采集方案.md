...
> **历史方案，已废止。** 本文仅用于追溯曾经的 Git Analytics 设计；不再要求采集、展示或恢复 Git 统计，不得据此重新启用扫描。现行决策与迁移验收见 [Git 统计移除与概览总 Token](./11-Git统计移除与概览总Token.md)。原有仓库管理、克隆同步、凭证、Git 身份、提交与变更审查功能仍保留。
