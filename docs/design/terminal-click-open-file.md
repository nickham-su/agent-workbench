...
- `provideLinks`:
  - 默认扫描单行文本,返回匹配到的 range+text.
  - 额外做两行硬换行启发式识别:
    - 仅支持两行拼接.
    - 第一行取行尾连续 token,且该 token 必须包含 `/`(看起来像路径).
    - 第二行允许行首空格/Tab 缩进,但必须从路径 token 开始,且同一行内包含 `:line`(不支持断在 `:` 的情况).
    - 拆分后的两段都会生成可点击链接,点击任一段都打开同一个目标文件.
  - 不做网络请求.
...
- 若路径在终端中被换行拆开,仅支持两行硬换行启发式的有限跨行识别(见 LinkProvider 设计),其他情况可能无法点击.
