...
- `main.ts`
  - 进程入口：加载 env、初始化 `DATA_DIR`、open DB、createApp、listen
- `app/`
...
- `modules/`
  - `health/`：`health.model.ts`、`health.service.ts`、`health.routes.ts`
...
- `AGENT_WORKBENCH_DATA_DIR=/data`
- `AGENT_WORKBENCH_HOME=/home/dev`
- `AGENT_WORKBENCH_LISTEN_HOST=127.0.0.1`
- `AGENT_WORKBENCH_PORT=...`
- `AGENT_WORKBENCH_FILE_MAX_BYTES=...`
  - 说明（当前实现）
    - 后端当前读取的变量为 `DATA_DIR`、`HOST`、`PORT`/`API_PORT`、`FILE_MAX_BYTES`
    - `AGENT_WORKBENCH_*` 属于未来 docker 交付形态下的建议命名，当前未接入
