# Assistant Markdown Render v1(基础+表格,无图片)
...
- Mermaid fenced code block 不做专门渲染:
  - 作为普通代码块展示
- 安全优先:
  - 禁止图片渲染(`![]()` 和 `<img>` 均不展示)
...
- Mermaid fenced code block 保持代码块展示:
  - 不做额外后处理渲染
...
- fenced code block ` ```mermaid ` 保持普通代码块展示,不做图形渲染.
...
- 避免每个 token/chunk 都触发一次完整 parse.
